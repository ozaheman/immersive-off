...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateScrumProgressBarHtml(scrumData) {
        // MODIFICATION START: Added Array.isArray check to prevent runtime errors
        if (!scrumData || !Array.isArray(scrumData.tasks) || scrumData.tasks.length === 0) {
            return '';
        }
    ...
    async function handleNewProject() {
        ...
        newProject.revNo = 'R0'; // Initial revision
    ...
    async function handleCreateRevision() {
        ...
        if (!confirm(`This will create a new revision (incrementing from ${project.revNo || 'R0'}). The current state will be archived as a PDF in the Documents section. Continue?`)) {
            return;
        }

        // 1. Snapshot current data from UI
        let uiData = {};
        Object.values(App.ProjectTabs).forEach(tabModule => {
            if (tabModule.getTabData) Object.assign(uiData, tabModule.getTabData());
        });
        const currentState = { ...project, ...uiData };

        // 2. Archive current state as PDF before incrementing
        const oldRev = project.revNo || 'R0';
        const revDate = App.DOMElements.revisionDate?.value || new Date().toISOString().split('T')[0];
        const activeTab = App.DOMElements.previewTabs?.querySelector('.tab-button.active');
        const previewId = activeTab ? activeTab.dataset.tab + "-preview" : "brief-proposal-preview";

        try {
            const pdfDataUrl = await PDFGenerator.generateDataUrl(previewId);
            await DB.addFile({
                jobNo: project.jobNo,
                source: 'master',
                category: 'scope_and_agreements',
                subCategory: 'old_revision_pdf',
                name: `Project_${project.jobNo.replace(/\//g, '-')}_Rev_${oldRev}_${revDate}.pdf`,
                fileType: 'application/pdf',
                dataUrl: pdfDataUrl,
                timestamp: new Date().toISOString()
            });
        } catch (e) {
            console.error("Failed to archive revision PDF:", e);
            if (!confirm("Failed to generate archival PDF. Proceed with revision increment anyway?")) return;
        }

        // 3. Increment Revision Number
        const currentRevNum = parseInt(oldRev.substring(1), 10) || 0;
        const nextRev = `R${currentRevNum + 1}`;
        ...
        project.revNo = nextRev;

        // Merge UI data into project
        Object.assign(project, uiData);
        project.revNo = nextRev; // Ensure it's the new one

        await DB.putProject(project);

        // 5. Update UI
        const revDisplay = document.getElementById('project-revision-display');
        if (revDisplay) revDisplay.textContent = `Rev: ${nextRev}`;

        App.ProjectTabs.Documents.renderAllGalleries(project.jobNo);
        alert(`Revision ${nextRev} created successfully. Previous state archived.`);
        Bulletin.log('Revision Created', `Project <strong>${project.jobNo}</strong> incremented to revision <strong>${nextRev}</strong>.`);
        await renderDashboard();
    }
    ...
    async function handleGeneratePdf() {
        ...
        // --- MODIFICATION: Automated Archival ---
        const archiveMap = {
            'tax-invoice': 'tax_invoice',
            'proforma': 'invoice',
            'receipt': 'receipt',
            'payment-certificate': 'payment_certificate'
        };

        if (archiveMap[activeTabKey]) {
            try {
                const pdfDataUrl = await PDFGenerator.generateDataUrl(previewId, App.DOMElements['page-size-selector'].value);
                await DB.addFile({
                    jobNo: App.currentProjectJobNo,
                    source: 'master',
                    category: 'financial_docs',
                    subCategory: archiveMap[activeTabKey],
                    name: `${fileName}_${new Date().toISOString().split('T')[0]}.pdf`,
                    fileType: 'application/pdf',
                    dataUrl: pdfDataUrl,
                    timestamp: new Date().toISOString()
                });
                App.ProjectTabs.Documents.renderAllGalleries(App.currentProjectJobNo);
                Bulletin.log('Document Archived', `<strong>${archiveMap[activeTabKey].replace(/_/g, ' ')}</strong> archived to Documents gallery.`);
            } catch (e) {
                console.error("Automated archival failed:", e);
            }
        }
    ...
    function setupEventListeners() {
        ...
        App.DOMElements['create-revision-btn']?.addEventListener('click', handleCreateRevision);
