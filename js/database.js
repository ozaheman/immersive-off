...
(function () { // Wrap in IIFE to avoid const redeclaration errors if loaded twice
    ...
    const publicAPI = {
        ...
        put: (storeName, data) => makeRequest(storeName, 'readwrite', store => {
            ...
            if (kp && (data[kp] === null || data[kp] === undefined)) {
                delete data[kp];
            }
            return store.put(data);
        }),
        add: (storeName, data) => makeRequest(storeName, 'readwrite', store => {
            const kp = store.keyPath;
            if (kp && (data[kp] === null || data[kp] === undefined)) {
                delete data[kp];
            }
        ...
        addRecentRecipient: async (recipient) => {
            ...
            const tx = db.transaction(STORES.RECIPIENT_HISTORY, 'readwrite');
            const store = tx.objectStore(STORES.RECIPIENT_HISTORY);
            const index = store.index('by_name');
            const request = index.get(recipient.name);
