document.addEventListener('DOMContentLoaded', () => {
    ...
    const HR_LETTER_TEMPLATES = {
        salary_certificate: (staff) => `
            ...
            <p>This is to certify that <b>${staff.name}</b>, holder of passport no. [Passport No.], has been employed with IMMERSIVE ENGINEERING CONSULTANTS L.L.C since ${formatDate(staff.joinDate)}.</p>
            ...
            <p>Sincerely,<br><b>For: IMMERSIVE ENGINEERING CONSULTANTS L.L.C</b></p>
        `,
    ...
    function renderHRLetterPreview() {
        ...
        if (!staff || !letterType) {
            DOMElements.hrLetterPreview.innerHTML = '<p style="text-align:center;color:#888;">Please select a staff member and a letter type.</p>';
        ...
        if (template) {
            DOMElements.hrLetterPreview.innerHTML = template(staff);
            DOMElements.downloadHrLetterPdfBtn.style.display = 'inline-block';
        } else {
            DOMElements.hrLetterPreview.innerHTML = '<p>Template not available for this letter type.</p>';
            DOMElements.downloadHrLetterPdfBtn.style.display = 'none';
        }
    }
    ...
    function setupEventListeners() {
        ...
        DOMElements['back-main-dashboard-btn']?.addEventListener('click', showDashboard);
    ...
    // --- FIX: Corrected handleTabSwitch function ---
    function handleTabSwitch(event) {
        ...
        // This is the key fix: it constructs the correct ID for control tabs.
