...
App.ProjectTabs.Invoicing = (() => {
    ...
    function setupEventListeners() {
        ...
        App.DOMElements.invoiceHistoryBody.addEventListener('click', e => {
            ...
            } else if (e.target.matches('.share-whatsapp-btn')) {
                const index = parseInt(e.target.closest('tr').dataset.index, 10);
                handleShareInvoice(index, 'WhatsApp');
            } else if (e.target.matches('.share-email-btn')) {
                const index = parseInt(e.target.closest('tr').dataset.index, 10);
                handleShareInvoice(index, 'Email');
            }
        });
            ...
            <tr data-index="${index}">
                ...
                <td>
                    <div style="display: flex; gap: 5px;">
                        <button class="view-invoice-link small-button">Preview</button>
                        <button class="share-whatsapp-btn small-button success" title="WhatsApp Share">WA</button>
                        <button class="share-email-btn small-button info" title="Email Share">Mail</button>
                    </div>
                </td>
            </tr>`;
        }).join('');
    ...
    async function handleShareInvoice(index, method) {
        const project = await DB.getProject(App.currentProjectJobNo);
        const inv = project.invoices[index];
        if (!inv) return;

        const clientName = project.clientName || 'Client';
        const clientMobile = project.clientMobile || '';
        const clientEmail = project.clientEmail || '';

        const subject = `Invoice ${inv.no} - Immersive`;
        const body = `Dear ${clientName},\n\nPlease find the details for Invoice ${inv.no}.\nAmount: ${App.formatCurrency(inv.total)}\nStatus: ${inv.status}\n\nKindly proceed with the payment.\n\nRegards,\nImmersive Team`;

        if (method === 'WhatsApp') {
            const phone = prompt("Enter WhatsApp phone number:", clientMobile.replace(/\D/g, ''));
            if (phone) {
                window.open(`https://wa.me/${phone}?text=${encodeURIComponent(body)}`, '_blank');
            }
        } else if (method === 'Email') {
            const email = prompt("Enter recipient email:", clientEmail);
            if (email) {
