...
// AppState is available via window.AppState (set in site_index.js)
...
export const ScheduleModule = {
    ...
    render: async (jobNo, containerId = 'villa-schedule-preview1') => {
        ...
        // Setup for simulation and BIM viewer
        setupSimulationControls(fullSchedule); // Use the full schedule for simulation range
        renderFloorSelector(); // Re-render selectors to update active state
...
export async function showNewTaskModal() {
    ...
    const { newTaskDependency } = window.DOMElements;
    if (newTaskDependency) {
        ...
        let siteData = await window.DB.getSiteData(window.AppState.currentJobNo);
        ...
        currentTasks.forEach(t => {
            newTaskDependency.innerHTML += `<option value="${t.id}">${t.name}</option>`;
        });
    }

    if (window.DOMElements.newTaskModal) window.DOMElements.newTaskModal.style.display = 'flex';
}

export async function saveNewTask() {
    const { newTaskName, newTaskDuration, newTaskFloor, newTaskDependency, newTaskModal } = window.DOMElements;
    const name = newTaskName.value;
    const dur = parseInt(newTaskDuration.value) || 5;
    const floor = newTaskFloor.value;
    const dep = newTaskDependency.value;

    if (!name) return alert("Task Name is required");

    let siteData = await window.DB.getSiteData(window.AppState.currentJobNo);
    if (!siteData) siteData = { jobNo: window.AppState.currentJobNo, customTasks: [], scheduleOverrides: [] };
    if (!siteData.customTasks) siteData.customTasks = [];

    const newId = 10000 + siteData.customTasks.length + 1;
    const newTask = {
        id: newId, name, duration: dur, floor,
...
export async function showNewTaskModalxxx() {
    if (!window.AppState.currentJobNo) return alert("Select a project first.");
    // alert('fffff1');
...
export async function saveNewTaskxxx() {
...
export async function getProjectSchedule(projectData, siteData) {
    ...
    const customTasks = (siteData && siteData.customTasks) ? siteData.customTasks : [];
    let allTemplates = [...VILLA_SCHEDULE_TEMPLATE, ...customTasks];
...
export function setupSimulationControls(schedule) {
    ...
    const slider = document.getElementById('simulation-slider');
    const dateDisplay = document.getElementById('simulation-date-display');
    const gotoCurrentDateBtn = document.getElementById('goto-current-date-btn');
    const playBtn1 = document.getElementById('play-simulation-btn1');
    const slider1 = document.getElementById('simulation-slider1');
    const dateDisplay1 = document.getElementById('simulation-date-display1');
    const gotoCurrentDateBtn1 = document.getElementById('goto-current-date-btn1');

    const sliders = [slider, slider1].filter(Boolean);
    const playBtns = [playBtn, playBtn1].filter(Boolean);
    const dateDisplays = [dateDisplay, dateDisplay1].filter(Boolean);
    const gotoBtns = [gotoCurrentDateBtn, gotoCurrentDateBtn1].filter(Boolean);
    ...
    // alert(window.BIMViewer);
    const updateViz = () => {
        const currentDays = parseInt(sliders[0].value);
        const currentDate = new Date(minTime + (currentDays * 24 * 60 * 60 * 1000));
        //dateDisplay.textContent = currentDate.toDateString();
        dateDisplays.forEach(display => display.textContent = currentDate.toDateString());
    ...
    sliders.forEach((slider, index) => {
        slider.oninput = () => {
            const otherSlider = sliders[1 - index];
            if (otherSlider) otherSlider.value = slider.value;
            updateViz();
        };
    });
    if (window.AppState.simulationInterval) clearInterval(window.AppState.simulationInterval);
    /* playBtn.onclick = () => {
    ...
    const togglePlay = () => {
        if (window.AppState.simulationInterval) {
            ...
            playBtns.forEach(btn => btn.textContent = "⏸ Pause");
            window.AppState.simulationInterval = setInterval(() => {
                let val = parseInt(sliders[0].value);
                if (val >= sliders[0].max) {
                    togglePlay(); // Stops the simulation
                } else {
                    sliders.forEach(slider => slider.value = val + 1);
                    updateViz();
                }
            }, window.AppState.simulationSpeed);
        }
    };

    const jumpToToday = () => {
        const today = new Date();
        const daysSinceStart = Math.ceil((today.getTime() - minTime) / (1000 * 60 * 60 * 24));
        const sliderValue = Math.max(0, Math.min(totalDays, daysSinceStart));
        sliders.forEach(slider => slider.value = sliderValue);
        updateViz();
    };

    playBtns.forEach(btn => btn.onclick = togglePlay);
    gotoBtns.forEach(btn => btn.onclick = jumpToToday);
...
export async function renderGanttChart() {
    ...
    let siteData = await window.DB.getSiteData(window.AppState.currentJobNo);
    // FIX: Handle undefined siteData
    if (!siteData) siteData = { jobNo: window.AppState.currentJobNo, scheduleOverrides: [], customTasks: [] };
